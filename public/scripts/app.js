...
(function () {
    ...
    PackagesController.$inject = ["$state", "$scope", "msUtils", "$mdDialog", "$document", "$q", "$compile", "NotesPackageService", "dxUtils", "authService", "firebaseUtils"];
    angular
        .module('app.notes.packages')
...
(function () {
    ...
    LanguagesController.$inject = ["$state", "$scope", "msUtils", "$mdDialog", "$document", "$q", "$compile", "NotesLanguageService", "dxUtils", "authService", "firebaseUtils"];
    angular
        .module('app.notes.languages')
            ...
            .state('app.notes.list', {
                ...
                views    : {
                 ...
                 resolve : {
                    ...
                    languages: ["adminService", function(adminService) {
                        return adminService.getLanguages();
                    }],
                    packages: ["adminService", function(adminService) {
                        return adminService.getPackages();
                    }]
                },
                bodyClass: 'notes'
            })
...
(function () {
    ...
    NotesController.$inject = ["$state", "$scope", "$timeout", "$compile", "$mdDialog", "$q", "$document", "authService", "firebaseUtils", "config", "msUtils", "dxUtils", "noteService", "languages", "packages"];
    angular
        .module('app.notes')
...
(function () {
    ...
    packageService.$inject = ["$firebaseArray", "$firebaseObject", "$q", "authService", "auth", "msUtils", "firebaseUtils", "dxUtils", "config"];
    angular
        .module('app.notes.packages')
...
(function () {
    ...
    languageService.$inject = ["$firebaseArray", "$firebaseObject", "$q", "authService", "auth", "msUtils", "firebaseUtils", "dxUtils", "config"];
    angular
        .module('app.notes.languages')
...
(function () {
    ...
    noteService.$inject = ["$rootScope", "$firebaseArray", "$firebaseObject", "$q", "authService", "auth", "firebaseUtils", "dxUtils", "config"];
    angular
        .module('app.notes')
...
(function () {
  ...
  LoginController.$inject = ["auth", "$state", "$firebaseObject", "authService", "$scope", "$timeout", "$mdToast"];
  angular
    .module('app.auth.login')
    .controller('LoginController', LoginController);

  /** @ngInject */
  function LoginController(auth, $state, $firebaseObject, authService, $scope, $timeout, $mdToast) {
    // Data
    var vm = this;
    // Methods
    vm.login = login;
    ...
    function login(loginForm) {
      auth.$signInWithEmailAndPassword(vm.form.email, vm.form.password)
        ...
        .catch(function (error) {
          ...
          $mdToast.show(
            $mdToast.simple()
              .textContent('Email or password not found')
              .position('top right')
              .hideDelay(3000)
          );
        });
...
(function () {
  ...
  LandingController.$inject = ["auth", "$state", "$q", "$firebaseObject", "authService", "$scope", "$timeout"];
  angular
...
(function () {
    ...
    function adminService($firebaseArray, $firebaseObject, auth, $q, $timeout, authService, firebaseUtils) {
        ...
        function getLanguages() {
            var defer = $q.defer(),
                ref = rootRef.child('tenant-notes-languages').orderByChild('deactivated').equalTo(null);
            firebaseUtils.fetchList(ref).then(function (data) {
                defer.resolve(data);
            });
            return defer.promise;
        ...
        function getPackages() {
            var defer = $q.defer(),
                ref = rootRef.child('tenant-notes-packages').orderByChild('deactivated').equalTo(null);
            firebaseUtils.fetchList(ref).then(function (data) {
                defer.resolve(data);
            });
            return defer.promise;
